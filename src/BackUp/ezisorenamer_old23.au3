...
#AutoIt3Wrapper_Outfile=EzISORenamer_test.exe
...
If $cmdline[0] <> 0 Then
	;verifica che non ci siano file iso e cartelle mischiate
	$iso = 0
	For $i = 1 To $cmdline[0]
		If StringRight($cmdline[$i], 4) = ".iso" Or ".ISO" Then $iso = $iso + 1
	Next

	Switch $iso
		Case $cmdline[0] ;sono tutti file .iso
			Startup()
			...
			ProgressOn($prog_name, "Starting batch renaming...")
			#region Batch Rename
			For $i = 1 To $cmdline[0]
				ProgressSet($i * 100 / $cmdline[0], "Analyzing """ & $cmdline[$i] & """")

				If FileMove($cmdline[$i], "*.iso", 1) = 0 Then
		LogWrite("ERROR! - File """ & $cmdline[$i] & """ - The file's extension cannot be renamed to .iso. Skipped.")
		$error = $error + 1
		ContinueLoop
	EndIf

	;vede se il file è nominato secondo il giusto formato
	Local $string = StringLeft($cmdline[$i], 12)
	...
	If $test = 0 Then
		Local $nome = StringLeft($cmdline[$i], StringLen($cmdline[$i]) - 4) ;nome del file non formattato, senza estensione

		;verifica se il nome è più corto di 32 caratteri
		If StringLen($nome) > 32 Then
			LogWrite("ERROR! - File """ & $cmdline[$i] & """ - The filename exceeds 32 characters and therefore has not been renamed. Shorten it and try again.")
			$error = $error + 1
			ContinueLoop
		EndIf
		...
		Switch $sles
			Case 0
				LogWrite("ERROR! - File """ & $cmdline[$i] & """ - The ID file cannot be extracted from the ISO. Make sure you have write access for the hard drive.")
				$error = $error + 1
				ContinueLoop
			Case Else
				$id = $sles[1]
				...
				If FileMove($cmdline[$i], $id & "." & $nome & ".iso") = 0 Then
					LogWrite("ERROR! - File """ & $cmdline[$i] & """ (" & $id & ") - The file cannot be renamed. Make sure you have write access for the hard drive or that the file is not open in another program or use the ID here to rename it manually.")
					$error = $error + 1
					ContinueLoop
				Else
					LogWrite("SUCCESS! - """ & $cmdline[$i] & """ - The file has been renamed correctly.")
				EndIf
		EndSwitch
	Else
		LogWrite("The file """ & $cmdline[$i] & """ seems to be named correctly.")
	EndIf
	...
	Sleep(250) ;sennò non si vede la progress bar
		...
		Case 0 ;non c'è nessun file .iso. Magari sono tutte cartelle?
			Local $a = 0
			For $i = 1 To $cmdline[0]
				$estensione = StringSplit($cmdline[$i], ".")
				If $estensione[$estensione[0]] = "iso" Or "ISO" Then $a = $a + 1
			Next

			If $a = $cmdline[0] Then ;sì, sono tutte cartelle
				Startup()
				For $i = 1 To $cmdline[0]
					Rename($cmdline[$i])
				Next

			Else ;no, ci sono file sconosciti
				MsgBox(16, "Error", "You are trying to process files that this program cannot handle." & @CRLF & @CRLF & "To use the Drag n Drop feature, drag here" & @CRLF & "- iso files" & @CRLF & "OR" & @CRLF & "- directories.")
				Exit
			EndIf

		Case Else ;il coglione ha messo roba mischiata. Ferma tutto
			MsgBox(16, "Error", "It seems you have dragged some iso files together with others." & @CRLF & @CRLF & "To use the Drag n Drop feature, drag here" & @CRLF & "- iso files" & @CRLF & "OR" & @CRLF & "- directories.")
			Exit
