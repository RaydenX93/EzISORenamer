...
If $cmdline[0] <> 0 Then
	...
	For $i = 1 To $cmdline[0]
		Local $TestPath = _PathSplit($cmdline[0], $szDrive, $szDir, $szFName, $szExt)
		...
		;If $TestPath[4] = "iso" or $TestPath[4] = "ISO" then $iso = $iso + 1
		;If StringRight($cmdline[$i], 4) = ".iso" Or StringRight($cmdline[$i], 4) = ".ISO" Then $iso = $iso + 1
	Next

	Select

;### Tidy Error -> "endselect" is closing previous "if" on line 24
	EndSelect
...
Switch $iso
	...
	Case 0 ;non c'è nessun file .iso. Magari sono tutte cartelle?
		...
		For $i = 1 To $cmdline[0]
			...
			If $estensione[$estensione[0]] = "iso" Or $estensione[$estensione[0]] = "ISO" Then $a = $a + 1
		Next
