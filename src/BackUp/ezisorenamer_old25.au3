...
If $cmdline[0] <> 0 Then
	...
	Switch $iso
		Case $cmdline[0] ;sono tutti file .iso
			...
			#region Batch Rename
			For $i = 1 To $cmdline[0]
				$nome_file_a = StringSplit($cmdline[$i], "\") ;VERIFICARE CON CARTELLA VUOTA/nonesistente CHE SUCCEDE
				$file = $nome_file_a[$nome_file_a[0]]

				ProgressSet($i * 100 / $cmdline[0], "Analyzing """ & $file & """")

				If FileMove($file, "*.iso", 1) = 0 Then
					LogWrite("ERROR! - File """ & $file & """ - The file's extension cannot be renamed to .iso. Skipped.")
					$error = $error + 1
					ContinueLoop
				EndIf
				...
				If $test = 0 Then
					...
					If StringLen($nome) > 32 Then
						LogWrite("ERROR! - File """ & $file & """ - The filename exceeds 32 characters and therefore has not been renamed. Shorten it and try again.")
						$error = $error + 1
						ContinueLoop
					EndIf

					;estrai il file magico (SLES_111.11, tipo) per l'identificazione
					ShellExecuteWait($sTempFolder & "\7z.exe", "e -y """ & $file & """ S??S_???.??", "", "open", @SW_HIDE)
					...
					Switch $sles
						...
						Case Else
							...
							If FileMove($file, $id & "." & $nome & ".iso") = 0 Then
...
Func Startup()
	...
	$logfile = FileOpen(@AutoItExe & "\" & $logfilename, 2)
	LogWrite("---------------------------------------------------------------------------------------------------")
