...
Func Rename($cartella)
	...
	$nome_cartella_a = StringSplit($cartella,"\")										;VERIFICARE CON CARTELLA VUOTA CHE SUCCEDE
	$nome_cartella = $nome_cartella_a[$nome_cartella_a - 1]
	...
	For $i = 1 To $iso_array[0]
		If $iso_array[0] <> 1 then ProgressSet($i * 100 / $iso_array[0], "Analyzing """ & $iso_array[$i] & """")
