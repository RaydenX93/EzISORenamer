...
;~ 					MsgBox(0, "", "Estensione maiuscola per " & $cmdline[$i] & @CRLF & FileMove($cmdline[$i], "*.iso", 1))
;~ 				EndIf
...
Func Rename($cartella)
	...
	For $i = 1 To $iso_array[0]
		...
		If StringRight($iso_array[$i], 3) == "ISO" Then
			MsgBox(0, "", "Estensione maiuscola per " & $iso_array[$i] & @CRLF & FileMove($iso_array[$i], "*.iso", 1))
		EndIf
		...
		Select
			...
			Case StringRight($file, 3) == "ISO"
				...
				Else
					LogWrite("SUCCESS! - """ & $file & """ - The file has been renamed correctly.")
;### Tidy Error -> "endselect" is closing previous "case" on line 274
Endif

			Case Else
				LogWrite("The file """ & $iso_array[$i] & """ seems to be named correctly.")
;### Tidy Error -> "endif" is closing previous "case" on line 316
			Endselect

			Sleep(250) ;sennò non si vede la progress bar
;### Tidy Error -> "next" is closing previous "select" on line 273
		Next
		ProgressOff()
		FileChangeDir("..")
		Return 1
;### Tidy Error -> "endfunc" is closing previous "for" on line 261
	EndFunc   ;==>Rename
