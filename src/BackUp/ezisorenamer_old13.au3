...
If $cmdline[0] <> 0 Then
	...
	For $i = 1 To $cmdline[0]
		If StringRight($cmdline[$i], 4) = "*.iso" Or StringRight($param, 4) = "*.ISO" Then $iso = $iso + 1
	Next
	...
	Switch $iso
		...
		Case 0 ;non c'è nessun file .iso. Magari sono tutte cartelle?
			...
			For $i = 1 To $cmdline[0]
				...
				If $estensione[$estensione[0]] = "iso" or "ISO" then $iso = $iso + 1
			Next

			If $iso = $cmdline[0] Then
				For $i = 1 To $cmdline[0]
			Rename($cmdline[$i])
		Next
			Else
		...
		Case Else ;il coglione ha messo roba mischiata. Ferma tutto
			MsgBox(16, "Error", "It seems you have dragged some iso files together with others." & @CRLF & "To use the Drag n Drop feature, drag here" & @CRLF & "- iso files" & @CRLF & "OR" & @CRLF & "- directories.")
	EndSwitch
