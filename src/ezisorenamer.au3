...
;controlla che ci sia una sola istanza del programma in corso
If _Singleton("eziso", 1) = 0 Then
	MsgBox(16, $prog_name, $prog_name & " is already running")
	Exit
EndIf

;Comportamento programma se vengono forniti dei parametri da linea di comando
If $cmdline[0] <> 0 Then
	...
	Switch $cmdline[0]
		...
		Case $iso + $empty ;iso e cartelle mischiate
			MsgBox(16, $prog_name, "You are trying to process iso files together with folders." & @CRLF & @CRLF & "To use the drag and drop feature, drag here" & @CRLF & "- iso files" & @CRLF & "OR" & @CRLF & "- directories containing iso files")
			Exit
		Case $iso + $empty + $unknown ;un mischione di tutto che non si può spiegare come
			MsgBox(16, $prog_name, "You are trying to process files and/or folders that this program cannot handle." & @CRLF & @CRLF & "To use the drag and drop feature, drag here" & @CRLF & "- iso files" & @CRLF & "OR" & @CRLF & "- directories containing iso files")
			Exit
		Case Else ;non ho idea di quando possa accadere una cosa del genere
			MsgBox(16, $prog_name, "Unexpected command line." & @CRLF & @CRLF & "To use the drag and drop feature, drag here" & @CRLF & "- iso files" & @CRLF & "OR" & @CRLF & "- directories containing iso files")
			Exit
	EndSwitch
...
;Comportamento programma standard
If FileExists("CD") = 0 And FileExists("DVD") = 0 Then
	MsgBox(16, $prog_name, "The program cannot find either the DVD folder nor the CD folder." & @CRLF & @CRLF & "You must place this program next to those two folders." & @CRLF & @CRLF & "Alternatively, you can drag and drop iso files or directories containing iso files onto the program's icon to process them.")
	Exit
Else
	If MsgBox(4 + 64, $prog_name, "Welcome to " & $prog_name & " v" & $version & " by RaydenX." & @CRLF & @CRLF & "This program will rename all your ISO files in the ""CD"" and ""DVD"" folders to OPL's required format." & @CRLF & @CRLF & "Do you want to start this task?" & @CRLF & @CRLF & "Alternatively, you can drag and drop iso files or directories containing iso files onto the program's icon to process them.") = 7 Then Exit
	Startup()
EndIf
...
Func Chiudi() ;Controlla che non ci siano stati errori e chiude il programma
	If $error <> 0 Then
...
Func Rename($cartella) ;rinomina tutti i file iso in $cartella
	...
	For $i = 1 To $iso_array[0]
		...
		Select
			Case $test = 0 ;se $test = 0, il file non segue il pattern ed è da rinominare
			...
			Case StringRight($iso_array[$i], 3) == "ISO" ;se il file segue il pattern ed ha solo l'estensione .ISO maiuscola
				If FileMove($iso_array[$i], "*.iso", 1) = 0 Then
					LogWrite("ERROR! - File """ & $iso_array[$i] & """ (" & $id & ") - The file cannot be renamed. Make sure you have write access for the hard drive or that the file is not open in another program or use the ID here to rename it manually.")
					$error = $error + 1
					ContinueLoop
				Else
...
Func LogWrite($log) ;crea un entry nel log
	FileWrite($logfile, "[" & @MDAY & "/" & @MON & "/" & @YEAR & " - " & @HOUR & ":" & @MIN & ":" & @SEC & "] " & $log & @CRLF)
EndFunc   ;==>LogWrite

Func Startup() ;sistema i file temporanei e avvia il log
...
Func BatchRename() ;rinomina tutti i file iso forniti nella linea di comando
	...
	For $i = 1 To $cmdline[0]
		...
		$file = $nome_file_a[$nome_file_a[0]] ;nomefile+estensione
		...
		;rinomina se necessario, se $test = 0 allora è da rinominare. Eventualmente, controlla se l'estensione del file è maiuscola e correggi.
		Select
			Case $test = 0
				...
				Switch $sles
					...
					Case Else
						...
						EndIf
				EndSwitch
			Case StringRight($file, 3) == "ISO"
				If FileMove($cmdline[$i], $orig_folder & "\*.iso", 1) = 0 Then
					LogWrite("ERROR! - File """ & $file & """ (" & $id & ") - The file cannot be renamed. Make sure you have write access for the hard drive or that the file is not open in another program or use the ID here to rename it manually.")
					$error = $error + 1
					ContinueLoop
				Else
